...
def test_upgrade_new_and_legacy_sqlite(tmp_path):
    ...
    with database.read() as db:
        assert run(db, "SELECT version_num FROM alembic_version").scalar_one() == "0014"
...
def test_copy_preserves_source_credentials_and_events(tmp_path):
    ...
    assert counts == {
        ...
        "model_calls": 0,
        "tenant_model_quotas": 0,
        "knowledge_vector_metadata": 0,
        "knowledge_index_checkpoints": 0,
        "knowledge_content_revisions": 0,
        "knowledge_index_batches": 0,
        "knowledge_index_scheduler": 0,
        "knowledge_index_workers": 0,
    }
...
def test_diagnostics_report_revision_without_parsing_model_data(tmp_path):
    ...
    upgrade(path)
    assert check(path)["revision"] == "0014"
