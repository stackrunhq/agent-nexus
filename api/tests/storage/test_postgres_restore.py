...
def test_backup_restore_data_auth_api_and_sequences(tmp_path):
    ...
    try:
        ...
        assert restored.check()["revision"] == "0017"
        ...
        with restored.engine.begin() as connection:
            ...
            connection.exec_driver_sql(
                f"GRANT SELECT, INSERT, UPDATE, DELETE ON models, model_audit, tenants, tenant_models, tenant_events, users, user_sessions, user_events, applications, application_versions, application_events, knowledge_documents, knowledge_chunks, knowledge_vector_indexes, knowledge_index_jobs, tenant_index_quotas, model_calls, tenant_model_quotas, knowledge_vector_metadata, knowledge_index_checkpoints, knowledge_content_revisions, knowledge_index_batches, knowledge_index_scheduler, knowledge_index_workers TO {runtime_role}"
            )
