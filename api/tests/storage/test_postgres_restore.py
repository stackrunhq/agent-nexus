...
def test_backup_restore_data_auth_api_and_sequences(tmp_path):
    ...
    try:
        ...
        assert restored.check()["revision"] == "0007"
        ...
        with restored.engine.begin() as connection:
            ...
            connection.exec_driver_sql(
                f"GRANT SELECT, INSERT, UPDATE, DELETE ON models, model_audit, tenants, tenant_models, tenant_events, users, user_sessions, user_events, applications, application_versions, application_events, knowledge_documents, knowledge_chunks, knowledge_vector_indexes, knowledge_index_jobs, tenant_index_quotas TO {runtime_role}"
            )
