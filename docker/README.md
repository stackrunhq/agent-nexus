...
**已有 0001/0002/0003/0004/0005 数据库须先备份并升级到 0009。** SQLite：暂停 API 和 Worker 后执行 `docker compose --project-directory . -f docker/compose.yaml run --rm --build api python -m agent_nexus.db_cli upgrade`，再启动 API 和 Worker；PostgreSQL 使用覆盖配置执行 migrate 服务，见 DATABASE.md。
...
新增 index-worker 服务，消费持久化索引任务；部署前升级至 0009。页面构建操作已改为提交后台任务，详见 [索引任务](../docs/INDEX_JOBS.md)。
