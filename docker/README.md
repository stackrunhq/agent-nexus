...
当前须升级 **0017**：备份并停止 API/全部 Worker，执行显式迁移后统一启动，仍为 24 表。见 [调用批次升级](../docs/CALL_BATCH_POSITION.md)；以下版本要求为历史记录。

当前须迁移至 **0016**，仍为 24 表。先备份并停止 API/全部 Worker，执行迁移后统一启动，新增调用任务关联列及索引。见 [升级说明](../docs/CALL_TASK_LINK.md)。下文旧版要求为历史记录。
