...
test('shows unknown usage and pages correlated calls',async()=>{
 ...
 const request=vi.spyOn(client,'request').mockResolvedValue({task:{id:'job',status:'failed',attempts:1,error:'provider_timeout',request_id:'req'},calls:{data:[{id:'call',status:'pending',error:null,elapsed_ms:null,input_tokens:null,output_tokens:0,association:'exact'},{id:'old',status:'succeeded',error:null,elapsed_ms:1,input_tokens:0,output_tokens:0,association:'request_match'}],has_more:true}} as never);
