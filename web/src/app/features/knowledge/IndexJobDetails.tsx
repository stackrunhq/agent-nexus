...
interface Call {id:string; status:string; error:string|null; elapsed_ms:number|null; input_tokens:number|null; output_tokens:number|null}
...
export function IndexJobDetails({client, root, id}: {client:AdminClient;root:string;id:string}) {
  ...
  return <section aria-label="索引任务详情">
    <h5>任务详情与同请求调用</h5>
    <p>按企业、请求 ID、模型和向量能力关联；请求 ID 重复时可能包含其他调用，不等同精确任务归属。无记录不代表未发生费用，调用成功也不代表索引构建成功。</p>
    ...
    {error ? <p role="alert">{error}</p> : !value ? <p>正在加载详情…</p> : <>
      ...
      {value.calls.data.map(call=><article key={call.id}>
        <p>调用 {call.id} · {call.status}{call.error ? ` · ${call.error}` : ''}</p>
