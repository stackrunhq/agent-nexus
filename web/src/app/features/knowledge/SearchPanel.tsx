...
export function SearchPanel({client, root, enabled}: {client: AdminClient; root: string; enabled: boolean}) {
  ...
  return <div><h4>检索已发布手册</h4><p>按关键词查找原文，支持中文、英文和数字。只搜索已发布文档，不生成大模型回答。</p>
    {error && <Alert type="error" message={error}/>}
    <Form layout="inline" onFinish={async ({query}) => {
      if (pending.current || !enabled) return;
      const controller = new AbortController(); pending.current = controller;
      setBusy(true); setError(''); setResults([]); setSearched(false);
      try {
        const result = await client.request<{data: Hit[]}>(root, 'POST', {query, limit:5}, controller.signal);
