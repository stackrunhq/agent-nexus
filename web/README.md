...
企业管理采用 React + TypeScript + Vite + Ant Design，入口 /admin/tenants；原模型工作台 /admin 暂保留原生页面，两者可互相跳转。切换页面需重新输入管理员令牌。
...
- src/app/shared/client.ts：管理员请求、204 空响应、错误和断开取消。
...
凭据仅在内存中；关闭凭据弹窗立即卸载内容，断开连接取消请求并清除页面数据。状态变更与轮换有确认；轮换后旧凭据立即失效。个人登录、角色与企业范围后台权限仍待开发。

企业页因 Ant Design 动态样式允许内联 CSS，脚本仍限定同源；原模型页 CSP 保持严格。当前生产 JS 约 948 KB（gzip 约 301 KB），后续页面扩展时做拆分加载。
