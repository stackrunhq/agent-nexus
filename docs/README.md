...
- [按尝试汇总调用](INDEX_ATTEMPT_SUMMARY.md)：全部精确关联调用统计、未知用量和范围说明。

- [调用尝试与批次定位](CALL_BATCH_POSITION.md)：当前 0017 升级、重试定位和未知字段说明。

- [调用任务关联](CALL_TASK_LINK.md)：当前 0016 升级、精确关联与未确认匹配。

- [任务详情与调用记录](INDEX_DETAILS.md)：管理员诊断接口、分页和关联边界。

- [检查点读写竞争](HISTORY_READ_WRITE.md)：混合读写、两连接池、提交后状态校验和复现方式。

- [多租户检查点查询实测](HISTORY_CONCURRENCY.md)：并发读取、检查点统计优化及原始计划。

- [历史查询实测](HISTORY_BENCHMARK.md)：执行计划、游标范围优化、原始结果及复现命令。

- [游标与查询索引](INDEX_CURSOR.md)：当前 0015 升级、游标续查、实时数据边界。

- [索引任务历史](INDEX_HISTORY.md)：服务端分页、模型/状态/错误码筛选及一致性边界。

- [索引失败诊断](INDEX_FAILURES.md)：失败原因筛选、处理建议、重试及历史范围。

- [Worker 心跳与配置观测](WORKER_HEARTBEAT.md)：当前 0014 升级、共享进程状态、策略不一致与运维边界。
