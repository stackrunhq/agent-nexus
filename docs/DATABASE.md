...
当前为 **0017**，24 表。model_calls 新增尝试/起始分片/批次大小三个可空整数列；停机备份后迁移，历史数据不回填。见 [当前升级](CALL_BATCH_POSITION.md)。

当前数据库为 **0016**，24 表；model_calls 新增可空 index_job_id 与企业/任务查询索引。停机备份后显式迁移并统一启动 API/Worker。见 [升级说明](CALL_TASK_LINK.md)，以下旧版要求为历史记录。

当前必须升级至 **0015**，仍为 24 表；增加任务历史排序索引，维护窗口暂停 API/Worker 后迁移。见 [当前升级](INDEX_CURSOR.md)，以下版本要求为历史记录。

当前版本 **0014**，共 **24 张业务表**，新增 `knowledge_index_workers`。先备份、停止 API 和全部 Worker，再显式升级并赋予运行角色新表读写权限。见 [当前升级说明](WORKER_HEARTBEAT.md)。以下保留历史迁移记录，旧版表数和版本号不代表当前要求。
...
- PostgreSQL 启动前必须迁移至 0010，缺少版本或版本不匹配时 API 拒绝启动。API 不自动执行生产迁移。
...
`/health/live` 仅表示进程存活；`/health/ready` 以零行查询核验全部业务表/字段和版本，不扫描或解析模型数据，失败返回 503 database_not_ready。它不验证模型服务、记录内容、数据库写权限或所有索引约束。SQLite 旧库允许 unversioned，已登记库只接受 0010。
