...
步骤 60（2026-09-16）新增可组合参数 job_id、actor，均为 1–200 字符、精确匹配；省略表示不限，空字符串返回 422。页面提交筛选或清除时回到最新页，输入草稿不会提前生效。操作者由 SQL 过滤；任务 ID 从通过格式和资源校验的历史载荷中匹配，损坏记录不能归属到指定任务。

任务筛选每次最多解析 1000 条候选事件，并最多返回 limit 条匹配；响应 scanned 表示检查数，scan_limit 表示解析上限。next_cursor 指向最后检查的事件，而非最后匹配事件，因此 data 为空且 next_cursor 非空时仍应继续。页面会明确提示继续查看更早记录。游标只是事件 ID，不绑定筛选；切换条件的客户端应清空游标。

存储评估：现有 application_events 只有主键和 application_id 单列索引，任务信息位于 action 文本，不能直接用普通索引查任务；强制将旧文本转换为数据库 JSON 会遇到坏记录及跨数据库兼容问题。本步保留 0017 和兼容读取，解析上限不约束数据库实际扫描行数，不视作性能优化成果。

下一步设计结构化导出审计存储及历史回填，评估 (application_id, version_id, job_id, id) 与操作者组合索引，通过真实执行计划再确定索引方案。
