...
当前版本 0017，备份恢复须包含 model_calls.index_attempt、index_batch_start、index_batch_size；旧数据保持空值。见 [字段与升级](CALL_BATCH_POSITION.md)。
...
0008 增加 model_calls，当前共 17 张业务表。恢复时保留 pending 记录，不能假设其未产生上游用量。运行角色需 model_calls 读写权限。

当前 0007 新增 tenant_index_quotas，共 16 张业务表；企业限额配置必须随业务数据备份恢复，运行角色需要该表的读写权限。

当前包含 knowledge_index_jobs，共 15 张业务表；备份和恢复需包含任务表。备份/迁移前停止 API、解析 Worker 和索引 Worker。恢复后的 processing 任务租约过期可重新领取，可能重复上游模型调用；已完成任务不会再次执行。
