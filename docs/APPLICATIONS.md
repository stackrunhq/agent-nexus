...
当前已提供 PDF/DOCX/Markdown/TXT 离线解析与来源分片，见 [知识库解析](KNOWLEDGE.md)。尚未提供文件上传、持久化、OCR 或文档检索。下一步将文件、分片及来源位置绑定到上述企业/应用/版本，加入后台任务和处理状态。产品版本发布和知识库内容发布需各自管理状态，不能混为一次操作。
