...
当前还没有文件上传、PDF/DOCX 解析、OCR、文本分片或文档检索。下一步先实现文本型 PDF、DOCX、Markdown、TXT 入库，并将文件、分片及来源位置绑定到上述企业/应用/版本；扫描 PDF 明确提示需要 OCR，后续再加入 OCR。产品版本发布和知识库内容发布需各自管理状态，不能混为一次操作。
